    ...
    >
      <div className="container mx-auto flex items-center justify-between px-5 md:px-20 py-8">
        ...
        <Link href="/" className="flex items-center">
          <div className={`${scrolled ? "text-[#E9967A]" : "text-[#E9967A]"} mr-2`}>
            <TbBrandGoogleHome size={38} className="stroke-1" />
          </div>
          <span className={`${scrolled ? "text-black" : "text-white"} font-semibold text-2xl`}>Desire Div</span>
        </Link>
        ...
        {show && (
          <nav className="fixed md:hidden inset-0 bg-[#0e1317] z-40 flex flex-col items-start px-8  justify-start py-32 space-y-8 text-xl ">
            ...
            <NavItem href="/about" label="About" mobile={true} setShow={setShow} />
            <NavItem href="/room" label="Room" mobile={true} setShow={setShow} />
            <NavItem href="/contact" label="Contact" simple mobile={true} setShow={setShow} />
          </nav>
        )}
